...
cd('C:\Users\johna\Downloads\Set2\Set2')

matD1 = dir('*.mat');
matD2 = {matD1.name};

for di = 1:length(matD2)
    load(matD2{di})
    
    plot(accel.accS1{1,1})
    hold on
    plot(accel.accS1{1,2})
    plot(accel.accS1{1,3})
    title(matD2{di},'interpreter','none')
...
%     YpointsTH = emgDat3(emgDat3 > tTh);
    xPoints = 1:length(emgDat3(:,e3));
    yPoints = emgDat3(:,e3);
    plot(xPoints(yPoints > tTh),yPoints(yPoints > tTh),'ro')
    hold on
